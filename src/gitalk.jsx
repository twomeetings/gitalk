...
class GitalkComponent extends Component {
	...
	createIssue () {
	  ...
	  // 当用户切换了页面，需要重新获取来刷新一下标题
	  const realTitle = window.document.title
	  return axiosGithub
	    .post(
	  ...
	  return (
	    <div className="gt-comments" key="comments">
	      ...
	      {!totalComments.length && (
	        <p className="gt-comments-null">{this.i18n.t('first-comment-person')}</p>
	      )}
	      ...
	      <span
	        ...
	        dangerouslySetInnerHTML={{
	          __html: this.i18n.t('counts', {
	            counts: `<a class="gt-link gt-link-counts" href="${issue &&
								issue.html_url}" target="_blank">${cnt}</a>`,
	            smart_count: cnt
	          })
	        }}
	      />
	...
	render () {
	  ...
	  return (
	    <div className={`gt-container${isInputFocused ? ' gt-input-focused' : ''}`}>
	      ...
	      {!isIniting && (isNoInit ? [this.noInit()] : [this.comments(), this.header()])}
	    </div>
	  )
	}
}
